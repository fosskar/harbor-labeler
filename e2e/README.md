...
flowchart TD
  ...
  P --> TL[chart run over TLS: pod pulled via<br/>https proxy, one TLS release per<br/>customCAs variant labels over https]
```
...
- **TLS with a private CA** (chart run over TLS stage) — chart releases
  talk to Harbor exclusively through an nginx TLS proxy (self-signed cert,
  generated by `e2e/gencert`, no openssl needed), one release per
  `customCAs` source variant: referenced ConfigMap, referenced Secret, and
  inline `certificates` (chart-rendered ConfigMap). Each exercises the CA
