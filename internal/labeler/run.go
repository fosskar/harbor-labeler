...
func Run(ctx context.Context, clusterName string, dryRun bool, discovery ImageDiscovery, harbor HarborAPI) error {
	images, err := discovery.RunningImages(ctx)
	if err != nil {
		return fmt.Errorf("discovering running images: %w", err)
	}

	if err := Reconcile(ctx, harbor, images, clusterName, dryRun); err != nil {
